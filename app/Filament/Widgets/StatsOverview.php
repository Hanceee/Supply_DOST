...
class StatsOverview extends BaseWidget
{
    ...
    protected function getCards(): array
    {
        ...
        $totalExpenses = Transaction::where('remarks', '!=', 'Cancelled')->sum('price');
        $totalTransactions = Transaction::where('remarks', '!=', 'Cancelled')->count();
        ...
        $highRatedSuppliersCount = Transaction::where('rating', '>=', 4.0)->where('remarks', '!=', 'Cancelled')->count();
        ...
        return [
            ...
            Card::make('Total Transactions this Year', Transaction::whereYear('date', now()->year)->where('remarks', '!=', 'Cancelled')->count())
                ->description('Total transactions this year')
                ->descriptionIcon('heroicon-s-calendar')
                ->chart($chartData->toArray())
                ->color('secondary'),
            Card::make('Total Transactions this Month', Transaction::whereMonth('date', now()->month)->where('remarks', '!=', 'Cancelled')->count())
